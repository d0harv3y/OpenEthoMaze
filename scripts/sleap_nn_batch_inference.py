...
Usage:
    python sleap_nn_batch_inference.py --model <model_dir> --video-dir <dir> [options]

Example:
    python sleap_nn_batch_inference.py --model ./models/my_run --video-dir "D:\videos" --batch-size 8

Running with uv (sleap-nn project uses uv):
    From sleap-nn repo dir, use its env to run this script (lives in IMPRESS):
        cd C:\\Users\\admin\\code\\sleap-nn
        uv run python "C:\\Users\\admin\\code\\IMPRESS\\my run inference\\sleap_nn_batch_inference.py" --model <model_dir> --video-dir <dir>
    Or from any dir, point uv at the sleap-nn project:
        uv run --project C:\\Users\\admin\\code\\sleap-nn python "C:\\Users\\admin\\code\\IMPRESS\\my run inference\\sleap_nn_batch_inference.py" --model <model_dir> --video-dir <dir>
    Ensure sleap-nn deps are synced first: cd sleap-nn && uv sync
"""

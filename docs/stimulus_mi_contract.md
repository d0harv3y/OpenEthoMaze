...
Stimulus duty is a **deterministic function of distance-to-exit**. Mutual information therefore conflates "response to delivered stimulus" with "response to goal proximity." No probe/omission trial exists in this cohort. The `wait` interval is a partial negative control; duty-vs-distance divergence is a partial saturation check only.

### ITI negative control — duty vs distance

| Interval | `stim_var` | Expected MI | If null is cleared |
|-------|------------|-------------|-------------------|
| `wait` | **duty** | ~chance (duty fixed off-center during wait) | Suspect join bug or mis-labeled interval |
| `wait` | **dist** | **May exceed null** | **Expected under chamber geometry**, not a failed control |
...
**Interpretation:** `wait_control_flag=1` with `stim_var=dist` means **distance MI is measuring goal/chamber geometry + locomotion**, not independent vibration coupling. Do not treat wait×dist as evidence of stimulus-driven behavior. Use **wait×duty** as the clean control; quarantine wait×dist from primary inference and apply the same caution to **run×dist** (duty and distance are coupled in run as well).
...
Cohort filters (CLI defaults): `experiment ∈ {VASTcont, VASTalt}`, drop `?`/blank strain and blank sex, optional `--condition`. Strains after drop must be `wt` or `tg`.

## Stage 2 — `stimulus_bin_edges.json`

Global fixed quantile edges (`--n-bins`, default 4) fit on **run-interval** bouts across the filtered cohort, separately for duty and distance. Reused when the sidecar exists.

## Stage 3 — `mi_per_animal.csv`

Per animal × interval (`run`, `wait`) × stim_var (`duty`, `dist`) × mi_type (`occupancy`, `transition`):

| Column | Description |
|--------|-------------|
| `animal_id`, `sex`, `strain`, `condition` | Stratification |
...
| `wait_control_flag` | 1 when wait `mi_mm` clears circular null (possible confound/bug) |
...
Per-animal `mi_mm` compared across `sex`, `genotype` (= `strain`), or `condition`:
...
Per animal × trial × interval × stim_var × mi_type (full factorial). Ordinal exposure columns support trajectory summaries.
...
| `cum_run_bouts` | Running sum of run-interval bouts through this trial (inclusive) |
| `interval`, `stim_var`, `mi_type` | Slice keys |
...
Per animal × interval × stim_var × mi_type trajectory scalars from per-trial rows:

...
- **S3 — Option A.** Mine candidate syllable sequences → exemplar tray → human-curated **per-fit rule table** (discover→curate) → apply grammar → S0 artifact. **DONE (code):** grammar mine/rules/export CLIs. **Curation S3.1:** documented in [`docs/grammar_rule_contract.md`](../grammar_rule_contract.md) (anchor_bucket, enrich-at-mine, overlay gates) — not implemented yet.

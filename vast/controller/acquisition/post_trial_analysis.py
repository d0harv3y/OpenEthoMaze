"""
Run VAST pipeline analysis (metrics, heatmap, movement bouts) for a single trial.
...
Requires vast.pipeline to be importable (e.g. PYTHONPATH including VAST, or
vast installed). If the import fails, run_analysis_for_trial returns
(False, "vast.pipeline not available").
...
def run_analysis_for_trial(
    ...
    """
    try:
        from vast.pipeline.io.file_discovery import TrialManifest
        from vast.pipeline.pipeline.process_trial import process_trial
    except ImportError:
        return (False, "vast.pipeline not available")

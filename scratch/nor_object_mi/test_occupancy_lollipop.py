...
def test_join_flags_da_hit_that_is_move() -> None:
    ...
    da = pd.DataFrame(
        {
            ...
            "step": ["no_obj->id_obj", "no_obj->id_obj"],
            "raw_syllable_id": [7, 8],
    ...
    row = s[(s["session"] == "NOR_BL") & (s["step"] == "no_obj->id_obj")].iloc[0]
...
def test_consensus_median_frac_across_models() -> None:
    ...
    row = c[(c["session"] == "NOR_BL") & (c["step"] == "id_obj->nvl_obj")].iloc[0]
...
def test_join_uses_destination_condition_occupancy() -> None:
    ...
    da = pd.DataFrame(
        {
            "model": ["m", "m"],
            "session": ["NOR_BL", "NOR_BL"],
            "step": ["no_obj->id_obj", "id_obj->nvl_obj"],
            "right": ["id_obj", "nvl_obj"],
            "raw_syllable_id": [7, 7],
    ...
    j = join_da_occupancy(da, occ_t, model="m")
    id_row = j[j["step"] == "no_obj->id_obj"].iloc[0]
    nv_row = j[j["step"] == "id_obj->nvl_obj"].iloc[0]
...
def test_signed_overlap_splits_gain_loss_by_occupancy() -> None:
    ...
    row = s[(s["session"] == "NOR_BL") & (s["step"] == "id_obj->nvl_obj")].iloc[0]

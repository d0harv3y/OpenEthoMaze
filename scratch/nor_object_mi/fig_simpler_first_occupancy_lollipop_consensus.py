...
FOOT = (
    ...
    "model × phase × condition). Presence uses id_obj occupancy; "
    "novelty/span use nvl_obj. Ids are not portable — pattern consensus, "
...
def fig_consensus(summary: pd.DataFrame, cons: pd.DataFrame, stem: Path, *, dest: str) -> None:
    ...
    for r, step in enumerate(STEPS):
        for c, ph in enumerate(SESSIONS):
            ax = axes[r][c]
            cell = summary[(summary["step"] == step) & (summary["session"] == ph)]
            ...
            if len(rec) == 1 and np.isfinite(float(rec["median_frac_da_move"].iloc[0])):
                ...
                ax.set_title(
                    f"{SESSION_SHORT.get(ph, ph)}  {n_maj}/{n_m} move>½",
                    loc="left",
                    fontsize=ts["annotation"] - 1,
                    color=INK,
                )
            else:
                ax.set_title(SESSION_SHORT.get(ph, ph), loc="left", fontsize=ts["annotation"] - 1, color=INK)
            ax.set_xticks([])
